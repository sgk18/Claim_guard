...
import { TextractOCRProvider } from "./ocr/textract";
...
// AWS credentials. Set it to "false" once AWS_REGION / AWS_TEXTRACT_REGION /
// AWS_BEDROCK_MODEL_ID are configured to switch to real Textract + Bedrock.
...
export function getOCRProvider(): OCRProvider {
  if (!ocrProvider) {
    ocrProvider = isMockMode() ? new MockOCRProvider() : new TextractOCRProvider();
  }
  return ocrProvider;
}
